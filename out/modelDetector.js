...
exports.ModelDetectorService = void 0;
...
class ModelDetectorService {
    constructor(context) {
        ...
        this.modelChangeEmitter = new vscode.EventEmitter();
        // Event for model changes
        this.onModelChange = this.modelChangeEmitter.event;
     ...
     */
    async detectFromChatContext(request) {
    ...
    async detectFromLMAPI() {
        ...
        try {
            ...
            if (!currentModel && availableModels.length > 0) {
                ...
                if (currentModel)
                    currentModel.accuracy = 'Available';
            }
            if (currentModel) {
    ...
    async detectFromStorage() {
        ...
        try {
            ...
            const cachedModel = this.context.globalState.get('currentModel');
            if (cachedModel) {
     ...
     */
    async detectCurrentModel(request) {
    ...
    addToHistory(model, method) {
        ...
        const limit = vscode.workspace.getConfiguration('aiModelDetector').get('historyLimit', 50);
    ...
    loadDetectionHistory() {
        const stored = this.context.globalState.get('detectionHistory', []);
    ...
    log(message, level = 'info') {
        ...
        if (enableLogging) {
            ...
            else if (level === 'warn' && config.get('showWarnings', false)) {
     ...
     */
    async scanDirectoryForModel(dirPath, maxDepth = 2) {
        try {
            if (maxDepth <= 0)
                return null;
     ...
     */
    async parseModelFromJsonFile(filePath) {
        try {
            ...
            if (!/^(copilot\/|gpt|claude|gemini|sonnet)/i.test(modelId))
                return null;
    ...
    inferVendor(modelId) {
        ...
        if (id.includes('copilot') || id.startsWith('gpt'))
            return 'OpenAI';
...
exports.ModelDetectorService = ModelDetectorService;
