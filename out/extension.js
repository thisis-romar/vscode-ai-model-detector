...
const modelDetector_1 = require("./modelDetector");
...
function activate(context) {
    ...
    const detectorService = new modelDetector_1.ModelDetectorService(context);
    const statusBarManager = new statusBar_1.StatusBarManager(detectorService);
    // 🌉 Initialize IPC Bridge for MCP integration (Phase 3)
    const ipcBridge = new ipcBridge_1.IPCBridgeServer(detectorService, 3001);
    ...
    const chatParticipant = new chatParticipant_1.ChatParticipantHandler(detectorService, ipcBridge);
    ...
    const commands = [
        ...
        vscode.commands.registerCommand('aiModelDetector.toggleStatusBar', async () => {
            ...
            const isEnabled = config.get('enableStatusBar', true);
            vscode.window.showInformationMessage(`Status bar monitoring ${isEnabled ? 'enabled' : 'disabled'}`);
        }),
        ...
        vscode.commands.registerCommand('aiModelDetector.showHistory', async () => {
            ...
            const panel = vscode.window.createWebviewPanel('aiModelHistory', 'AI Model Detection History', vscode.ViewColumn.One, {
                enableScripts: true
            });
    ...
    context.subscriptions.push(participant, statusBarManager, detectorService);
